...
static char temperature_threshold[100];
static int temperature_threshold_len = 0;

static char position[100];
static int position_len = 0;
...
#define INITIAL_TEMPERATURE "100C"
#define INITIAL_POSITION  "Awaiting coordiantes in form x:00.000, y:000.000, alt:000"

/*
 * The resource handler
 */
static void
hnd_temperature_get(coap_resource_t *resource,
                    ...
                    coap_pdu_t *response)
{
    ...
    coap_add_data_large_response(resource, session, request, response,
                                 ...
                                 (size_t)temperature_threshold_len,
                                 (const u_char *)temperature_threshold,
                                 NULL, NULL);
}

static void
hnd_temperature_put(coap_resource_t *resource,
                  coap_session_t *session,
                  const coap_pdu_t *request,
                  const coap_string_t *query,
                  coap_pdu_t *response)
{
    ...
    if (strcmp (temperature_threshold, INITIAL_TEMPERATURE) == 0) {
    ...
    if (size == 0) {      /* re-init */
        snprintf(temperature_threshold, sizeof(temperature_threshold), INITIAL_TEMPERATURE);
        temperature_threshold_len = strlen(temperature_threshold);
    } else {
        temperature_threshold_len = size > sizeof (temperature_threshold) ? sizeof (temperature_threshold) : size;
        memcpy (temperature_threshold, data, temperature_threshold_len);
    }
}

static void
hnd_position_get(coap_resource_t *resource,
                    coap_session_t *session,
                    const coap_pdu_t *request,
                    const coap_string_t *query,
                    coap_pdu_t *response)
{
    ...
    coap_add_data_large_response(resource, session, request, response,
                                 query, COAP_MEDIATYPE_TEXT_PLAIN, 60, 0,
                                 (size_t)position_len,
                                 (const u_char *)position,
                                 NULL, NULL);
}

static void
hnd_position_put(coap_resource_t *resource,
                  coap_session_t *session,
                  const coap_pdu_t *request,
                  const coap_string_t *query,
                  coap_pdu_t *response)
{
    ...
    if (strcmp (position, INITIAL_POSITION) == 0) {
        coap_pdu_set_code(response, COAP_RESPONSE_CODE_CREATED);
    } else {
        coap_pdu_set_code(response, COAP_RESPONSE_CODE_CHANGED);
    }
    ...
    if (size == 0) {      /* re-init */
        snprintf(position, sizeof(position), INITIAL_POSITION);
        position_len = strlen(position);
    } else {
        position_len = size > sizeof (position) ? sizeof (position) : size;
        memcpy (position, data, position_len);
    }
}

static void
hnd_CameraStatus_get(coap_resource_t *resource,
                    coap_session_t *session,
                    const coap_pdu_t *request,
                    const coap_string_t *query,
                    coap_pdu_t *response)
{
    int CameraStatus_len;
    
    CameraStatus_len = strlen(CameraStatusMessages[CameraCounter]);
    coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
    coap_add_data_large_response(resource, session, request, response,
                                 query, COAP_MEDIATYPE_TEXT_PLAIN, 60, 0,
                                 (size_t)CameraStatus_len,
                                 (const u_char *)CameraStatusMessages[CameraCounter],
                                 NULL, NULL);
...
static void
hnd_BatteryStatus_get(coap_resource_t *resource,
                    coap_session_t *session,
                    const coap_pdu_t *request,
                    const coap_string_t *query,
                    coap_pdu_t *response)
{
    int BatteryStatus_len;
    
    BatteryStatus_len = strlen(BatteryStatusMessages[BatteryCounter]);
    coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
    coap_add_data_large_response(resource, session, request, response,
                                 query, COAP_MEDIATYPE_TEXT_PLAIN, 60, 0,
                                 (size_t)BatteryStatus_len,
                                 (const u_char *)BatteryStatusMessages[BatteryCounter],
                                 NULL, NULL);
...
static void coap_example_server(void *p)
{
    ...
    coap_resource_t *resource_position = NULL;
    coap_resource_t *resource_camera = NULL;
    ...
    //Set initial position to zeros, waiting for new values...
    snprintf(position, sizeof(position), INITIAL_POSITION);
    position_len = strlen(position);
    ...
    while (1) {
        ...
        resource_temperature = coap_resource_init(coap_make_str_const("Temperature"), 0);
        if (!resource_temperature) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        coap_register_handler(resource_temperature, COAP_REQUEST_GET, hnd_temperature_get);
        coap_register_handler(resource_temperature, COAP_REQUEST_PUT, hnd_temperature_put);
        coap_resource_set_get_observable(resource_temperature, 1);
        coap_add_resource(ctx, resource_temperature);

        resource_position = coap_resource_init(coap_make_str_const("Position"), 0);
        if (!resource_position) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        coap_register_handler(resource_position, COAP_REQUEST_GET, hnd_position_get);
        coap_register_handler(resource_position, COAP_REQUEST_PUT, hnd_position_put);
        coap_resource_set_get_observable(resource_position, 1);
        coap_add_resource(ctx, resource_position);

        resource_camera = coap_resource_init(coap_make_str_const("CameraStatus"), 0);
        if (!resource_camera) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        coap_register_handler(resource_camera, COAP_REQUEST_GET, hnd_CameraStatus_get);
        coap_resource_set_get_observable(resource_camera, 1);
        coap_add_resource(ctx, resource_camera);

        resource_battery = coap_resource_init(coap_make_str_const("BatteryStatus"), 0);
        if (!resource_battery) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        coap_register_handler(resource_battery, COAP_REQUEST_GET, hnd_BatteryStatus_get);
